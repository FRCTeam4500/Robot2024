...
public class CommandSwerve extends SwerveDrive {
            ...
            ()
              ...
              if (Math.abs(xbox.getRightY()) > 0.5) {
                targetAngle = Rotation2d.fromDegrees(
                    90 - 90 * Math.signum(-xbox.getRightY()));
              }
              targetAngle = Rotation2d.fromDegrees(
                  targetAngle.getDegrees() - xbox.getRightX() * rotationalSens);
              driveAngleCentric(-xbox.getLeftY() * forwardSens,
                                -xbox.getLeftX() * sidewaysSens, targetAngle);
            },
            this)
        .beforeStarting(() -> {
          targetAngle = getRobotAngle();
    ...
    return Commands
        .run(
            ()
              ...
              driveRobotCentric(
                  new ChassisSpeeds(-xbox.getLeftY() * forwardSens,
    ...
    return Commands
        .run(
            ()
              ...
              driveAlignToTarget(-xbox.getLeftY() * forwardSens,
                                 -xbox.getLeftX() * sidewaysSens, targetAngle);
            },
            this)
        .beforeStarting(() -> {
  ...
  public Command driveToTag(Pose2d targetPose) {
    return Commands.run(() -> {
      Pose2d poseDif =
          tagVision.getRelativeTagPose(targetPose).relativeTo(targetPose);
      driveRobotCentric(
          new ChassisSpeeds(poseDif.getX() * 2, poseDif.getY() * 2,
  ...
  public Command driveToPiece() {
    return Commands.run(() -> {
      if (!pieceVision.seesPiece()) {
        ...
        driveRobotCentric(new ChassisSpeeds(ExtendedMath.clamp(0, 1, 20 - area),
                                            5 * diff.getRadians(), 0));
      }
    }, this);
  }
  ...
  public void initSendable(SendableBuilder builder) {
    ...
    builder.addDoubleProperty("Target Angle (Deg)",
                              () -> targetAngle.getDegrees(), null);
  }
}
