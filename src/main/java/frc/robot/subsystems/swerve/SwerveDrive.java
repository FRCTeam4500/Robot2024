...
public class SwerveDrive extends SubsystemBase implements LoggableInputs {
  ...
  protected SwerveDrive() {
    ...
    odometry = new SwerveDriveOdometry(kinematics, gyro.getUnwrappedAngle(),
                                       getModulePositions(),
                                       tagVision.getRobotPose(new Pose2d()));
    poseEstimator = new SwerveDrivePoseEstimator(
        kinematics, gyro.getUnwrappedAngle(), getModulePositions(),
        tagVision.getRobotPose(new Pose2d()));
  ...
  public boolean gyroConnected() { return gyro.getAHRS().isConnected(); }
  ...
  public void periodic() {
    ...
    if (tagVision.seesTag()) {
      poseEstimator.addVisionMeasurement(
          tagVision.getRobotPose(getEstimatorPose()), Timer.getFPGATimestamp());
    }
  }

  public void driveAngleCentric(double forwardVelocity, double sidewaysVelocity,
                                Rotation2d targetRotation) {
    driveRobotCentric(ChassisSpeeds.fromFieldRelativeSpeeds(
        forwardVelocity, sidewaysVelocity,
        calculateRotationalVelocityToTarget(targetRotation), getRobotAngle()));
  }

  public void driveAlignToTarget(double forwardVelocity, double leftVelocity,
                                 Rotation2d aligningAngle) {
    driveRobotCentric(new ChassisSpeeds(
        ChassisSpeeds
            .fromFieldRelativeSpeeds(forwardVelocity, leftVelocity, 0,
                                     getRobotAngle())
            .vxMetersPerSecond,
        pieceVision.getHorizontalOffset(new Rotation2d()).getDegrees() /
            (pieceVision.getTakenArea(99)),
  ...
  public void driveRobotCentric(ChassisSpeeds targetChassisSpeeds) {
    ...
    SwerveDriveKinematics.desaturateWheelSpeeds(states, MAX_LINEAR_SPEED_MPS);
  ...
  public void resetPose(Pose2d newPose) {
    odometry.resetPosition(gyro.getUnwrappedAngle(), getModulePositions(),
                           newPose);
    poseEstimator.resetPosition(gyro.getUnwrappedAngle(), getModulePositions(),
                                newPose);
  }

  public Rotation2d getRobotAngle() { return gyro.getOffsetedAngle(); }

  public void zeroRobotAngle() { gyro.zero(); }
  ...
  public void initSendable(SendableBuilder builder) {
    builder.addDoubleProperty("Offset Robot Angle (deg)",
                              () -> gyro.getOffsetedAngle().getDegrees(), null);
    builder.addDoubleProperty("Forward Velocity (mps)",
                              () -> getChassisSpeeds().vxMetersPerSecond, null);
    builder.addDoubleProperty("Sideways Velocity (mps)",
                              () -> getChassisSpeeds().vyMetersPerSecond, null);
    builder.addDoubleProperty(
        "Rotational Velocity (radps)",
        () -> getChassisSpeeds().omegaRadiansPerSecond, null);
    // builder.addBooleanProperty("Gyro Connected", () ->
    // gyro.getAHRS().isConnected(), null);
  }

  private double
  calculateRotationalVelocityToTarget(Rotation2d targetRotation) {
    double rotationalVelocity = anglePID.calculate(getRobotAngle().getRadians(),
